...
function convert() {
    ...
    const number = parseInt((document.getElementById('input') as HTMLInputElement).value);
    ...
    let gammaFuncCheckBox = (document.getElementById('gamma-function') as HTMLInputElement).checked;
    let eulersIdentityCheckBox = (document.getElementById('eulers-identity') as HTMLInputElement)
        .checked;
    let limitExponentialCheckBox = (document.getElementById(
        'limits-exponential'
    ) as HTMLInputElement).checked;
    let limitPolynomialCheckBox = (document.getElementById('limits-polynomial') as HTMLInputElement)
        .checked;
    let trigCheckBox = (document.getElementById('trig') as HTMLInputElement).checked;
    let geometricSeriesCheckBox = (document.getElementById('geometric-series') as HTMLInputElement)
        .checked;
    ...
    function limit_polynomial(n: number) {
        ...
        // https://en.wikipedia.org/wiki/List_of_limits#Functions_of_the_form_xg(x)
            ...
            let numerator_length = Math.floor(Math.random() * highest_power - 1) + 1;
            let denominator_length = Math.floor(Math.random() * highest_power - 1) + 1;
            let signs = ['-', '+'];
    ...
    function trig_identity(n: number) {
    ...
    function decompose(n: number) {
        ...
        function decompose2(n: number) {
            ...
            if (diff1 < diff2) {
                ...
                return `{ \\left({${randomOption1(randNum)}}\\right)${power} + {${randomOption2(
                    difference
                )}}}`;
            } else {
                let power = `^{${ceilLog}}`;
        ...
        function minExp(n: number) {
            ...
            if (diff1 < diff2) {
                ...
                oldDiff = diff2;
                exp = <[number, string]>[exp2, `\\left({${randomOption3(3)}}\\right)^{${ceilLog}}`];
                valObj[oldDiff] = exp;
            }
        ...
        if (n !== 0 && n < 100 && (n === 2 || !isPrime(n)) && randomValue < 0.25) {
            ...
            // ab
            if (Math.random() < 0.2) {
                return `{{\\left({${randomOption1(a)}}\\right)}{\\left({${randomOption2(
                    b
                )}}\\right)}}`;
            }
